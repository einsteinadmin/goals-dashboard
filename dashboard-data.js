  ...
  },
  {
   ...
   "goals": [
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "6/23: Green, active. Brian ran his Vera scrum-scorecard skill yesterday — pulled all branches (Sergio, Houston, N. Austin, S. Austin). Still tweaking how it reads the boards. Built a 'best practices expectations' doc for managers from the output. DoD: get Vera running scrum scorecards in prep sync ups to automate grading. Planning to migrate to a single QG pulse per BM next quarter (instead of individual task pulses) for cleaner weekly status visibility.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "6/23: Green, tracking Super Green. 3 done. Needs S. Austin + Houston. Will likely hit S. Austin next week (short week but possible). On pace for 5 total.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "yellow",
     "assessment": "7/2 huddle: Fixing the Training Day app + DB going out so next week's Training Day is organized. Legend-of-the-Week rotation codified: branch legends on TD weeks, leadership-level legends rotate weekly across departments (Brian updating the pulse).",
    ...
    },
    {
     ...
     "green": "Read by 5/30",
     "assessment": "6/23: Super Green — in Section 4 now (page 91), just met Ted. Steady pace."
    }
   ]
  },
  {
   ...
   "goals": [
    {
     ...
     "critical": true,
     "status": "supergreen",
     "assessment": "6/23: Jalissa onboarded + taking recruiting (Anne refrains unless tagged). This week: teach her Rippling + Monday (DoD = she understands our Monday processes). Daily morning meetings in place; may delegate board automations to Linda. SG locked.",
    ...
    },
    {
     ...
     "assessment": "7/2 huddle: Repivot underway — met Paul earlier this week, working the transcript through Claude; Jalissa handoffs starting (culture interview + draft competency interviews). 2pm working session w/ Cameron today covers mover promotion + comp philosophy + gates."
    },
    {
   ...
   "goals": [
    {
     ...
     "critical": true,
     "status": "green",
     "assessment": "7/2 huddle: Tet + Nhel turned loose on the coaching process (their tweaks under review). Open gates: % of REAL sales calls graded is unknown (Cameron's gate: 100% graded / 80% = green) + rubric reweighting timing; Matisen's idea to tie estimate-vs-actual variance into grading adopted as a direction. Cameron tagged Amanda on the pulse.",
    ...
    },
    {
     ...
     "critical": true,
     "status": "green",
     "assessment": "6/23: ⭐ Botpress contract SIGNED + Antonin's questions answered. Billing starts within 7 days. DoD: schedule + run the implementation meeting. Trending Super Green if rolled out by end of quarter (close). ⚠ Watch: un-disable-able Auto-Recharge = the cost-explosion risk to nail during implementation.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "6/23: → Green (from Yellow). Copy done; gate is Matisen's token + MJML/HTML decision (~6/25 target). DoD: check in w/ KP + circle up w/ Matisen. Blocked on IT.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "6/23: First Class Precision running strong; held at Green until an outcome metric (not just leaderboard activity) surfaces.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "yellow",
     "assessment": "6/23: → Yellow (from Red). Chipping away; book done by end of June.",
     "superGreen": "Read by EOQ + 1-page reflection on personal H/H/S gaps",
     "green": "Read by 5/30"
    }
   ]
