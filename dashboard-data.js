const DASHBOARD_DATA = {
 "meta": {
  ...
  "lastRefreshed": "2026-08-31T14:30:00-05:00",
  "refreshedBy": "Albert (digest \u2014 Anne sync + O3)",
  "archive": {
   "quarter": "Q2 2026",
  ...
  "statusHistoryStamped": "2026-08-31"
 },
 "network": {
  "phase": "1",
 ...
 "cameronKPIs": [
  {
   ...
   "target": "66%",
   "actual": "8/31 hub: running-truck 38.8% (Aug, pulled 8/29) \u00b7 trailing-active 85.5% (Q3, pulled 8/29). Ruler mismatch w/ Matisen still open \u2014 not graded vs 66%.",
   "status": "unknown",
  ...
  },
  {
   ...
   "target": "20%",
   "actual": "8/31 hub: Conversion $ (slide-back) 23.9% vs 20% target (pulled 8/29) \u2014 Green",
   "status": "green",
  ...
  },
  {
   "name": "Customer NPS",
   "target": "90+",
   "actual": "8/31 hub: Aug NPS 86.4 (GatherUp direct, pulled 8/24) \u2014 below 90 = Red on binary ruling",
   "status": "red",
 ...
 "cameronRocks": [
  ...
  },
  {
   ...
   "critical": false,
   "status": "green",
   "assessment": "8/28 BARS AMENDED (Cameron approved): the original SG 'launched by 10/31' was written before anyone knew Secret Stache's build clock. Rich told Laura 8/26 \u2014 start date 8/31, 10-11 weeks to their final review, 13 weeks is the launch goal (~Nov 30), tracking ahead on recent projects, gantt chart lands week 2-3. A 10/31 launch sits outside the vendor's own range, so Q3's finish line is now a BUILD milestone and LAUNCH moves to a Q4 rock. Bars stay outcome-based (no-caveat grading holds); Red fires if Einstein is the blocker. KP goal #1 amended to the same language so the same work isn't graded two ways. State: all site copy pre-approved by Cameron; Lianna's website + email copy is the remaining review; build starts 8/31.",
  ...
  },
  {
   ...
   "critical": false,
   "status": "green",
   "assessment": "8/28 PROMOTED BONUS \u2192 ROCK #5 and BARS REWRITTEN (both Cameron-approved). The carried-over bonus bars were already substantially met \u2014 \"engagement kicked off + playbook delivered\" both happened in August \u2014 which would have made this an auto-Green with two months left. New bars measure the REMAINING Q3 scope: rubric built and in use, cohorts actually running, per-rep reporting live. SG keeps the lead-triage/EMMA plan, the one piece genuinely still ahead. Live state: five moves locked with weights \u00b7 call flow CONFIRMED by Jeff 8/27 \u00b7 roleplay app v1.8 in trial with Nhel + Tet \u00b7 abbreviated class w/ Jeff Thu 9/3 12:30 to build the rubric \u00b7 cohorts resume ~wk of 9/8 \u00b7 weekly conversion reporting pulsed to Amanda + Matisen. \u26a0 Distinct from Amanda Ware's Rock #6, which grades the OUTCOME (close rate +2/+3.5 pts off the frozen 8/5 baseline). This is the sponsor/architect lane.",
   ...
   "goals": [
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "via Vera 8/24: Two halves to this rock and the manager-roundtable half just moved. The first manager strategy meeting of the quarter is set for Thursday 9/3, 2:30-3:45, all 10 branch managers across both regions, and the agenda has been redesigned rather than repeated \u2014 a pair-first format where managers brief each other in pairs, then each presents their partner's notes to the room, with leadership split into two leads and two silent observers. It is built directly against the failure mode these sessions have had: managers vocal offline and silent in the room, handing problems up instead of solving them. Blueprint is published and the plan is posted to the rock pulse; two format calls are still with Mike and Cameron. On the events half, budget is closed at $75 per person and options and candidate dates are compiled across 5 regions and 37 attendees, but no region has voted and no date is locked yet.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "via Vera 8/24: Peer review is complete and the first cut is made. 94 deduped peer ratings across all four branches; on-site evaluation averages pulled from the FM Super Board. Scoring model rebuilt 8/25 \u2014 Application dropped to weight 0 (reference only, because the cover-letter instruction never reached the application link), leaving Peer 30 / On-Site 30 / Games 15. A published cut line is now in force: a candidate must average 4.0 or higher on the core-values peer score to reach the projects round. Ben Perez (3.67) and Jeremy Ecker (3.62) did not clear it and are not advancing \u2014 the first defensible, criteria-based cut this process has had. Jake Petersen runs the projects round Friday 8/28; all four projects are defined with prompts, time windows, and submission rules, and the candidate email is reconciled to match. Remaining candidates get the same package next.",
    ...
    },
    {
     ...
     "critical": true,
     "status": "green",
     "assessment": "via Vera 8/24: Rollout shipped 8/24 \u2014 all 15 managers have their board and the announcement is on the Delegation Board. Two managers are certified: Calvin Hughes 52/60 and Joel Villatoro 54/60 after same-day rework, both re-scored under rubric v1.5. Twelve boards remain. Deadline extended 8/27 from Thursday 8/27 to Monday 8/31 \u2014 v1.5 landed mid-cohort on 8/26 and forced the re-score of the first two certs, so holding the rest to Thursday would have graded them against a bar that moved under them. Also corrected a live error in the manager announcement: the last two categories were labeled Bonus when v1.5 makes them 5 points each, 10 of the 60 \u2014 anyone who skipped them could not have cleared the 51-point line. A defect was found in the certification quiz \u2014 two Paragraph-type questions Google Forms cannot auto-grade, holding 2 of 10 points at zero. The 7 responses received were hand-graded 8/27; the underlying question-type fix is still open.",
  ...
  },
  {
   ...
   "goals": [
    ...
    },
    {
     ...
     "critical": true,
     "status": "yellow",
     "assessment": "8/31 (sync): DoD this week = run weighting through the consultants council + Replit weight-distribution survey; mockup on pace for EOW ~9/4. | via Linda 8/27: 10 decisions locked from the 7/3 design workshop (Cameron + Anne + Paul); damages mechanism now has real clarity via Cameron's 7/30 QGR ruling (in the promotion gate AND the bonus). \u26a0\ufe0f 8/27 (Cameron 1:1): formally confirmed this is NOT a Q3 deliverable \u2014 full system launch targeted January 1, 2027, timed to a 'new year, new performance philosophy' narrative alongside the new handbook, since it's a foundational change (attendance/damages point weights, reset cadence) that needs real branch input, not a rushed Q3 ship. Q3's realistic deliverable is the weighting-input tool: Anne will first run the existing framework through the /consultants council (Cameron confirmed Linda doesn't have this skill yet \u2014 he's sharing it) to refine it, then build a Replit survey where BMs/movers distribute 100 points across weighted metrics (same pattern Cameron used for the branch scorecard at QGR), synthesize the averages, and use disagreements as the workshop agenda.",
   ...
   "goals": [
    {
     ...
     "critical": true,
     "status": "yellow",
     "assessment": "via Gary 8/31: Yellow \u2014 the join is finally answered; the cadence itself just slipped. CoS-MH1 answered the 7/3 Emma-integration ask on 8/21 (6 weeks of silence) with a workable plan: match calls to jobs via the Monday 'Scheduled Jobs \u2014 Q3 2026' tracker (board 18427242877) on Customer Phone. Amanda confirmed 8/25 it's a plumbing change, not a rebuild \u2014 the grader already captures external_number, just needs to carry it through by call_id. But grading itself stalled: no run happened the week of 8/18-8/24, and there's no confirmed run since 8/17 as of 8/31 \u2014 a real cadence gap Amanda owns closing, not just a data question. Company average still 45.8 cumulative against a 65 Green bar; no new score data to move that number one way or the other since 8/17.",
   ...
   "goals": [
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "8/28 CONFIRMED by Cameron, with #1 AMENDED to match his Rock #2 verbatim. Laura's draft Green ('ready to go, but waiting for new year to launch') had the right timeline instinct but no date in it. Secret Stache starts 8/31; 13-week goal = ~Nov 30 launch.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "8/28 CONFIRMED as drafted. Launch ~wk of 9/21; Fabian on paternity leave (3 dev projects paused, QA continues). Origin/destination-for-inestimables logic tabled until post-rollout (~9/28).",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "8/28 CONFIRMED as drafted. \u26a0 Austin L's own goals carry harder numbers (48-hr SLA, 10+ pieces/month, 4 long-form published) \u2014 grade him against his bars, not this one. Austin L now reports to Amanda Ware; KP keeps managing the pipeline.",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "8/28 CONFIRMED as drafted. Overlaps Amanda Ware's Rock #3 (extend the email design system to the full suite) \u2014 Cameron ruled the two pushing on each other is fine, no owner split. Template #1 coded; MJML\u2192HTML copy-paste for the rest (Matisen).",
    ...
    },
    {
     ...
     "critical": false,
     "status": "green",
     "assessment": "8/28 CONFIRMED as drafted. \u26a0 No volume or performance number in the bar \u2014 Cameron ruled it stands (five goals is already a full load for KP; this one reads close to an auto-win by design). Sarah's finals due to Lamar 8/31; extended-truck concept held for the next round.",
